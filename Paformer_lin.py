...
class ParallelLayer(nn.Module):
    def __init__(self, args, out_dim=None):
        ...
        self.out_dim = out_dim if out_dim is not None else args.dim
        self.Share = Linear(args.dim * args.n_parallel_blocks, self.out_dim)
        
    def forward(self, x, mask, freqs):
        outputs = [block(x, mask, freqs) for block in self.blocks]
        # Concatenate outputs along last dim
        outputs = torch.cat(outputs, dim=-1)
        return self.Share(outputs)
